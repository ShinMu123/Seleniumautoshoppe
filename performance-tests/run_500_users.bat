@echo off
REM Run JMeter non-GUI load test for 500 concurrent users and generate HTML report.
...
if exist "results\menu_500_html" rmdir /s /q "results\menu_500_html"

"%JMETER_EXE%" -n -t menu_api_load_test.jmx -l results\menu_500.jtl -e -o results\menu_500_html ^
...
echo [INFO] Done. Report: results\menu_500_html\index.html
endlocal
